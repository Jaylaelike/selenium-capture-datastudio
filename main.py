...
def handle_request():
    ...
    if captured_status is not None:
        ...
        data_date = time.strftime("%d/%m/%Y ", time.localtime(time.time()))
        notify.send(f"📊 📈รายงานสถิติไฟฟ้าดับศูนย์วิศวกรรมชุมพร ประจำสัปดาห์ {data_date}", image_path=captured_image_path)
